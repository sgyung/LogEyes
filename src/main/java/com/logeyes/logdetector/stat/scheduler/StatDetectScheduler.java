...
//    }
    ...
    public void run() {
        ...
        LocalDateTime bucket =
                ErrorFingerprintStat5m.normalizeTo5MinBucket(now.minusMinutes(1));

        log.info("[SCHEDULER] detect start bucket={}", bucket);

        statAlertDetectorService.detect(bucket);

        log.info("[SCHEDULER] detect end bucket={}", bucket);
    }
}
